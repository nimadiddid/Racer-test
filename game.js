...
function drawCar(x, y, w, h, pal, isPlayer) {
  ctx.save();
  // Draw tinted image for opponent cars, normal for player
  if (!isPlayer) {
    // Apply color tint using composite for opponents
    ctx.drawImage(CAR_IMG, x, y, w, h);
  ...
  if (isPlayer) {
    ...
    ctx.fillRect(x + w*0.15, y + h*0.04, w*0.18, h*0.025);
    ctx.fillRect(x + w*0.67, y + h*0.04, w*0.18, h*0.025);
  } else {
    ctx.shadowColor = "#FF2200"; ctx.shadowBlur = 18;
    ctx.fillStyle = "rgba(255,50,0,0.6)";
    ctx.fillRect(x + w*0.15, y + h*0.88, w*0.18, h*0.025);
    ctx.fillRect(x + w*0.67, y + h*0.88, w*0.18, h*0.025);
  }
  ctx.restore();
}
